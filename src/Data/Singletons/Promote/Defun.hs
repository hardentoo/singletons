...
buildDefunSyms (DClassD _cxt name tvbs _fundeps _members) = do
  defunctionalize name tvbs (Just (DConT constraintName))
...
buildDefunSymsTypeFamilyHead default_tvb default_kind
  ...
  defunctionalize name arg_tvbs res_kind

buildDefunSymsTySynD :: Name -> [DTyVarBndr] -> PrM [DDec]
buildDefunSymsTySynD name tvbs =
  defunctionalize name tvbs Nothing
...
buildDefunSymsDataD ctors =
  ...
  where
    ...
    promoteCtor ctor@(DCon _ _ _ _ res_ty) = do
      ...
      defunctionalize name arg_tvbs (Just res_ki)
...
defunctionalize :: Name -> [DTyVarBndr] -> Maybe DKind -> PrM [DDec]
defunctionalize name m_arg_tvbs' m_res_kind' = do
